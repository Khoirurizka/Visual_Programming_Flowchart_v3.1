...
function ScrewDriverFrameRender() {
  ...
  useEffect(() => {
    ipcRenderer.on("bev_driver_capture", (event, File_image) => {
      try {
    ...
    return () => {
      ipcRenderer.removeAllListeners("bev_capture");
    };
  }, []);
    ...
    >
      {frameReceived ? (
        ...
        <p style={{ textAlign: 'center', fontSize: '0.9rem' }}>
          The image is currently unavailable. Please wait for the BEV robot to capture it.
        </p>
      )}
    </div>
  );
